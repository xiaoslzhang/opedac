...
import numpy as np
from load import papers, authors, confs
...
def get_conf_info(conf_name):
    ...
    conf_papers_two_years_ago = list(
        filter(lambda paper: paper["year"] <= 2011, conf_papers))
...
def get_features(author_name, head=False):
    ...
    feature_head += ',' + \
        ','.join(list(map(lambda x: str(x) + '_cnt', range(1994, 2014))))
    feature_head += ',' + \
        ','.join(list(map(lambda x: str(x) + '_citation', range(1994, 2014))))
    feature_head += ',' + \
    ...
    feature_head += ',' + \
        ','.join(list(map(lambda x: 'first_author_' +
                          str(x) + '_cnt', range(1994, 2014))))
    feature_head += ',' + \
        ','.join(list(map(lambda x:  'first_author_' +
                          str(x) + '_citation', range(1994, 2014))))
    ...
    features['career_age'] = 2013 - author_papers[0]["year"] + 1
    ...
    author_papers_in_five_year = list(
        filter(lambda paper: paper["year"] > 2008, author_papers))
    ...
    # 合作作者的各种特征
    def cal_rank(_author_name=author_name, start_year=1936, end_year=2013):
        _paper_list = list(
            map(lambda p_index: papers[p_index], authors[author_name]["papers"]))
        ...
        if len(_filter_paper_list) == 0:
            ...
            _career_age = 2013 - _paper_list[0]["year"] + 1
            _range_career_age = _career_age - (2013 - end_year)
            _citation_ = sum(
    ...
    if len(coauthors) != 0: 
        ...
        coauthors_delta = list(map(lambda _author_name: cal_rank(
            _author_name=_author_name) - cal_rank(_author_name=_author_name, end_year=2011), coauthors))
